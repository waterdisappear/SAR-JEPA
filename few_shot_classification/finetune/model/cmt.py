...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 10, 'input_size': (1, 128, 128), 'pool_size': None,
...
class CMT(nn.Module):
    def __init__(self, img_size=128, in_chans=1, num_classes=10, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=1280,
        ...
        if representation_size:
            ...
            self.pre_logits = nn.Sequential(OrderedDict([
                ('fc', nn.Linear(embed_dims[-1], representation_size)),
                ('act', nn.Tanh())
            ]))
        else:
