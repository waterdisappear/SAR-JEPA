...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False) -> nn.Sequential:
    ...
    for v in cfg:
        if v == "M":
            ...
            if batch_norm:
                layers += [padding, conv2d, nn.BatchNorm2d(v), nn.ReLU(inplace=True)]
            else:
                layers += [padding, conv2d, nn.ReLU(inplace=True)]
            in_channels = v
...
class MVGGNet(torch.nn.Module):
    def __init__(self, num_classes=3):
        ...
        self.classifier1 = nn.Sequential(nn.BatchNorm1d(512*4*4, affine=False, eps=1e-6), nn.Linear(512 * 4 * 4, 256))
